...
func showPlayerError(chNames: LadderPlayer) throws {
    guard chNames.names.count != 1 else {
        print("플레이어를 두사람이상 입력해주세요")
        print("오류가 발생했으니 게임을 다시 실행해주세요")
        throw UserInputError.incorrectPlayerData
    }
    var largeNameCount = 0
    for index in 0..<chNames.names.count {
        if largeNameCount < chNames.names[index].count {
            largeNameCount = chNames.names[index].count
