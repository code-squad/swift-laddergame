...
while run {
    ...
    for idx in 0..<inputPlayer.count {
        ...
        while true {
            ...
            guard heightOfLadder > 1 && heightOfLadder < 11 else { print("사다리 높이는 2~10까지 가능합니다."); continue }
            guard let laddergame = LadderGame.init(heightOfLadder, inputPlayer) else { break }
